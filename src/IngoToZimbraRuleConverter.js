...
class IngoToZimbraRuleConverter {
    ...
    static validConditionFilter(condition) {
        return condition.field !== '' && condition.value !== '' && !['regex', 'less', 'greater'].includes(condition.match);
    };
    ...
    static conditionValue(condition) {
        return condition.field.toLowerCase() === 'size' ? condition.value.toUpperCase().replace(/[B\s]/g, '') : condition.value;
    }
